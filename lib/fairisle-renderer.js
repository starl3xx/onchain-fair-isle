...
// Rare multicolor palette (~5% chance)
...
function renderFairIsle(seed) {
  ...
  // Determine if rare (5% chance) - use LCG to distribute evenly across all seeds
  // Grandfather tokens 0-3 as Nordic Rainbow (minted before fix)
  const rarityCheck = ((Math.abs(seedNum) + 1) * 16807) % 100;
  const isRare = seedNum <= 3 || rarityCheck < 5;
