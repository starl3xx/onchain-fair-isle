...
/**
 * Everything a Fair Isle is, short of the markup: the 100x100 grid of colour
 * indices plus the traits derived from the band sequence. Split out of
 * renderFairIsle so rasterizers can blit stitch tiles by grid index instead of
 * building (and parsing) an 8 MB, 60,000-element SVG. Both paths share this
 * function, so the grid can never disagree with the SVG.
 * @param {bigint|number|string} seed - The seed value
 * @returns {{ grid: number[][], palette: object, paletteIndex: number, isRare: boolean, hasGiantSnowflake: boolean }}
 */
function renderFairIslePattern(seed) {
  ...
  // Whether the sequence actually landed the giant snowflake center band.
  // This is the ground truth — the draw happens mid-sequence after many RNG
  // advances, so it cannot be reproduced by any closed-form check on the seed.
  const hasGiantSnowflake = bands.some((item) => item.band === GIANT_SNOWFLAKE);
...
function renderFairIsle(seed) {
  const pattern = renderFairIslePattern(seed);
 ...
 * Ink stays strictly inside the cell, so a grid of these tiles rasterizes
 * identically to the same stitches inside the full-size SVG.
 * @param {string} color - Base colour of the stitch
 * @param {number} px - Rendered tile size in pixels
 * @returns {string}
 */
function renderStitchTileSVG(color, px) {
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${STITCH_W} ${STITCH_H}" width="${px}" height="${px}">${renderStitch(0, 0, color, STITCH_W, STITCH_H)}</svg>`;
}
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = { renderFairIsle, renderFairIslePattern, renderStitchTileSVG, renderFairIsleDataURI, PALETTES, GRID_COLS: COLS, GRID_ROWS: ROWS };
}
